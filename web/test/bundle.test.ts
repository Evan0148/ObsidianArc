...
// shell, the terminal, the Chinese dictionary and the LaTeX renderer. Those
// four are the only splits this project has, and each is deliberate — a
// quarter of the code most accounts can never reach, a panel every account
// may open and few ever will, a dictionary only half the readers need, and a
// renderer only a maths answer wakes.
describe('code splitting invariants in production bundle', () => {
  ...
  it('contains the expected chunk files without extra fragments', () => {
    ...
    // Exactly six, and deliberately exact: the project ships no fonts, no
    // images and no other chunks, so a seventh file is either a split that
    ...
    // into every account's menu.
    expect(files, `unexpected build output: ${files.join(', ')}`).toHaveLength(6);
