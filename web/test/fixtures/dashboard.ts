...
export const emptyTotals: UsageTotals = {
  ...
  total_tokens: 0, credits: 0, errors: 0, users: 0, models: 0, duration_ms: 0,
};
...
export function dashboardFixture(): Dashboard {
  ...
  return {
    ...
    newest_users: [],
    last_24h: { ...emptyTotals, requests: 4906, input_tokens: 166700000, output_tokens: 1900000, total_tokens: 168600000, credits: 2000000, errors: 1071, users: 64, models: 12, duration_ms: 4906 * 8400 },
    prev_24h: { ...emptyTotals, requests: 4460, input_tokens: 150000000, output_tokens: 1700000, total_tokens: 151700000, credits: 1800000, errors: 820, users: 58, models: 11, duration_ms: 4460 * 9100 },
    ...
    top_models: ['Claude Sonnet 4.5', 'GPT-5', 'Gemini 2.5 Pro', 'DeepSeek V3', 'Qwen3', 'Other model'].map((label, i) => ({
      ...emptyTotals, key: `model-${i}`, label, detail: ['Anthropic', 'OpenAI', 'Google', 'DeepSeek', 'Alibaba', 'Other'][i]!,
      requests: 1500 - i * 230, total_tokens: 12000000 - i * 1900000, credits: [840000, 640000, 340000, 126000, 64000, 42000][i]!,
      users: 40 - i * 6, models: 1, last_at: Date.now() - i * 60000,
    })),
    top_users: ['onyx', 'lin', 'momo', 'alex', 'ada'].map((label, i) => ({
      ...emptyTotals, key: `user-${i}`, label, detail: label, requests: 1200 - i * 200, total_tokens: 9000000 - i * 1000000, credits: 450000 - i * 70000,
      users: 1, models: 3, last_at: Date.now() - i * 60000,
    })),
    series, bucket_ms: 21600000,
    heatmap: [{ weekday: 4, hour: 21, requests: 64, total_tokens: 780000 }, { weekday: 1, hour: 10, requests: 12, total_tokens: 150000 }],
