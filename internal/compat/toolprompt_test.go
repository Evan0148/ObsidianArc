...
// toollessFixture is the ordinary fixture with emulation switched on for
// the model, which is the row an operator sets after watching an endpoint
// swallow the tools field.
func toollessFixture(t *testing.T) *fixture {
	t.Helper()
	f := newFixture(t)
	on := true
	if _, err := f.models.Update(context.Background(), f.model.ID,
		model.Update{EmulateTools: &on}); err != nil {
		t.Fatal(err)
	}
	return f
}
...
func TestTheEmulatedLoopCompletes(t *testing.T) {
	...
	if !strings.Contains(answer, `<tool_response tool="read_file">`) || !strings.Contains(answer, "package main") {
