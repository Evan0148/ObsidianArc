...
// toollessModel is a spec whose endpoint drops tool fields — the row an
// operator marks after watching a model answer `tools` with prose. Not
// supports_tools, which describes the model and is false on plenty of
// models that call tools natively.
func toollessModel() ModelSpec {
	spec := testModel()
	spec.EmulateTools = true
	return spec
}
...
func TestEmulatedRequestTradesToolFieldsForInstructions(t *testing.T) {
	...
	if !strings.Contains(answer, "a language") || !strings.Contains(answer, `<tool_response tool="lookup">`) {
