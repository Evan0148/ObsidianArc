...
| --- | --- |
| 二进制体积 / Binary size | 21.0 MB（Linux amd64；使用 `-tags nosqlite` 为 17.3 MB） |
| 冷启动就绪时间 / Cold start | ~28 ms |
| 空闲内存占用 / Idle RSS | ~16 MB |
| 20 并发流式峰值 / Peak under 20 concurrency | ~54 MB 内存，11 个 OS 线程 |
| 首次加载传输体积 / Wire payload | 打开对话界面传输 178.63 kB（148.69 kB JS + 29.94 kB CSS）；中文语言包 (31.76 kB)、管理后台 (78.11 kB)、终端 (7.20 kB)、公式渲染器 (3.61 kB) 按需分包加载 |
